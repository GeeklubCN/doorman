...
func (f *identifier) Identify(code string) (core.Identification, bool) {
	...
	if err != nil {
		logrus.Errorf("get token fail! code:%s. %v", code, err)
		return "", false
	}
	var t oauthToken
	err = json.Unmarshal(body, &t)
	if err != nil {
		logrus.Errorf("token illegal! body:%s. %v", body, err)
		return "", false
	}
	userInfo, err := f.api.getUserInfo(t.getAuthorization())
	if err != nil {
		logrus.Errorf("get user info fail! auth:%s. %v", t.getAuthorization(), err)
		return "", false
	}
